...
class CartPoleEvaluator:
    ...
    def evaluate(self, num_episodes=500):
        ...
        for ep in range(num_episodes):
            ...
            for t in range(500):
                if t < self.seq_len:
                    # Use PID controller for first `seq_len` steps
                    action = self._pid_controller(state)
                else:
                    # Use Decision Transformer after `seq_len` steps
                ...
                current_ep['angles'].append(next_state[2])  # Pole angle
                current_ep['cart_velocities'].append(next_state[1])  # Cart velocity
                current_ep['cart_positions'].append(next_state[0])  # Cart position
    ...
    def _print_statistics(self, results):
        ...
        print(f"Success Rate (500 steps): {(durations == 300).sum() / len(durations):.1%}")
...
if __name__ == "__main__":
    ...
    evaluator = CartPoleEvaluator(model_path=model_location, seq_len=20)
